...
field_scale = 'scale'; value_scale = num2cell(ones(1,l_Cand));

noise = struct(field_type, value_type, field_para, value_para, field_scale, value_scale);
...
Para_apgl.Clambda = [0.8, 0.4, 0.4, 0.4, 0.4, 0.4]; % Gaussian, Cauchy, Lognormal

Para_l2.Clambda = [1.5, 1.5, 1.5, 1.5, 1.5, 1];
%tuningPara_l2.Lf = [2e4, 2e4, 2e4];

Para_shrink.Clambda = [0.8, 0.8, 0.7, 0.7, 0.65, 0.65];
Para_shrink.Ctau = [1.5, 1.5, 1.5, 1.5, 1.5, 1.5];

Para_l1.Clambda = [0.9, 0.9, 0.9, 0.9, 0.9, 0.9];
%tuningPara_l1.Lf = [3e4, 3e4, 3e4];

Para_wilcoxon.Clambda = [0.35, 0.3, 0.3, 0.3, 0.3, 0.3];
wilcoxonLf = [5e3 5e4 5e4 5e5 3e4 3e4];
...
for noiseInd = 1 : 6
    ...
    for trial = 1 : niter
        ...
        [X, y, Theta_star] = DataGen(nr, nc, N, r, noise(noiseInd), 'MR', 'small_asymmetric');
        ...
        if running(1)
            ...
            Record(1, 1, trial, noiseInd) = norm(Theta_hat1-Theta_star, 'fro')^2/(nr*nc);
            Record(1, 2, trial, noiseInd) = rank;
            Record(1, 3, trial, noiseInd) = elapsedTime;
        ...
        if running(2)
            ...
            Record(2, 1, trial, noiseInd) = norm(Theta_hat2-Theta_star, 'fro')^2/(nr*nc);
            Record(2, 2, trial, noiseInd) = rank;
            Record(2, 3, trial, noiseInd) = elapsedTime;
        ...
        if running(3)
            ...
            Record(3, 1, trial, noiseInd) = norm(Theta_hat3-Theta_star, 'fro')^2/(nr*nc);
            Record(3, 2, trial, noiseInd) = rank;
            Record(3, 3, trial, noiseInd) = elapsedTime;
        ...
        if running(4)
            ...
            Record(4, 1, trial, noiseInd) = norm(Theta_hat4-Theta_star, 'fro')^2/(nr*nc);
            Record(4, 2, trial, noiseInd) = rank;
            Record(4, 3, trial, noiseInd) = elapsedTime;
        ...
        if running(5)
            ...
            Record(5, 1, trial, noiseInd) = norm(Theta_hat5-Theta_star, 'fro')^2/(nr*nc);
            Record(5, 2, trial, noiseInd) = rank;
            Record(5, 3, trial, noiseInd) = elapsedTime;
        ...
        if running(6)
            ...
            Record(6, 1, trial, noiseInd) = norm(Theta_hat6-Theta_star, 'fro')^2/(nr*nc);
            Record(6, 2, trial, noiseInd) = rank;
            Record(6, 3, trial, noiseInd) = elapsedTime;
...
if save_flag
    Record_MR_small_tn_d40 = Record;
    save('highN.mat', 'Record_MR_small_tn_d40');
end

%disp(num2str(log(Record(:,1,2,:)*nr*nc)/2))% method, record(loss, rank, time), niter, noise(t(2^k))
