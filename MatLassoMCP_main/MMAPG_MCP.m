...
function [Theta_hat, rank, outInfo] = MMAPG_MCP(y, X, type, Clambda, tol, maxiter, Theta_init, penalty_In)
...
elseif nargin == 8
    penalty = penalty_In;
    lambda = Clambda * sqrt(log(nr * nc)*N);
end
    ...
    switch penalty
        ...
        case 'l1'
            if strcmp(type.name, 'Wilcoxon')
                [~,weight_new] = sort(y-Sampling_new);
                [~,weight_new] = sort(weight_new);
                obj      = sum((weight_new/(N-1)-0.5*(N+1)/(N-1)).*(y - Sampling_new)) + lambda_run .* sum(sum(abs(Theta_new)));
                obj_line = sum((weight/(N-1)-0.5*(N+1)/(N-1)).*(y-Sampling)) + sum(sum(Grad .* (Theta_new - Theta_nold))) + ...
                    (tau/2) * sum(sum((Theta_new-Theta_nold).^2)) + lambda_run .* sum(sum(abs(Theta_new))); % sum(sum((Theta_new-Theta_old).^2))
            else
                obj      = sum(Loss(y - Sampling_new)) + lambda_run .* sum(sum(abs(Theta_new)));
                obj_line = sum(Loss(y-Sampling)) + sum(sum(Grad .* (Theta_new - Theta_nold))) + ...
