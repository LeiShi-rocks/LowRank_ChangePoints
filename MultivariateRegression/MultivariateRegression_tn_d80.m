...
Record = zeros(6, 3, niter, l_Cand); % method, record(loss, rank, time), niter, noise(t(2^k))
...
if save_flag
    Record_MR_small_tn_d80 = Record;
    save('Record_MR_small_tn_d80.mat', 'Record_MR_small_tn_d80');
end

%disp(num2str(log(Record(:,1,1,:)*6400)/2))% method, record(loss, rank, time), niter, noise(t(2^k))
