...
load('/Users/leishi/Documents/GitHub/LowRank_ChangePoints/MatLassoMCP_main/Simulations/MR_SCP_large_signal/record_N_MR_SCP_large_signal.mat');
...
running_flag = [0,0,0,1];

num_iter = 100;
record_N_refit = record_N;
% 4 methods: ours, LASSO, Oracle, NC
% [Fnormsq_1, rank1], [Fnormsq_2, rank2], tau_hat, obj_path, Delta_path, Theta_hat,
...
for N_ind = 1:length(N_Cand)
    ...
    for iter = 1:num_iter
        ...
        if running_flag(1)
            ...
            [Theta_hat_1, rank1] = MVAPG_MCP(y(:,first_seg), X(:,first_seg), type, Clambda, tol, maxiter, Theta_init);
            [Theta_hat_2, rank2] = MVAPG_MCP(y(:,second_seg), X(:,second_seg), type, Clambda, tol, maxiter, Theta_init);
        ...
        if running_flag(2)
            ...
            [Theta_hat_1, rank1] = MVAPG_MCP(y(:,first_seg), X(:,first_seg), type, Clambda, tol, maxiter, Theta_init, 'l1');
            [Theta_hat_2, rank2] = MVAPG_MCP(y(:,second_seg), X(:,second_seg), type, Clambda, tol, maxiter, Theta_init, 'l1');
...
if save_flag
    save('record_N_refit_MR_SCP_large_signal.mat', 'record_N_refit');
end


%% varying dimension
clear;clc;
load('record_dim_MR_SCP_large_signal.mat');
fprintf('Running: MR_SCP_LARGE_SIGNAL, varying dimension... \n\n')
%load('record_dim_refit_MR_SCP_large_signal.mat');
dim_Cand = [25, 50, 75];
...
if save_flag
    save('record_dim_refit_MR_SCP_large_signal.mat', 'record_dim_refit');
end
